...
if(isset($_POST['delete'])){
	...
	if(!(empty($eid))){
		$q="select * from `cetaevent` where id='$eid'";
		...
		//echo @mysqli_num_rows($qr);
			...
			$evenue=$row['venue'];
			$etype=$row['type'];
			$qi="INSERT INTO `pastevents`(`id`, `link`, `name`, `type`, `date`, `venue`, `time`) VALUES (NULL,'$r','$ename','$etype','$edate','$evenue','$etime')";
			$qrr=mysqli_query($mysqlc,$qi);
				
		
	    
		$q="delete from `cetaevent` where `id`='$eid'";
		$qr=@mysqli_query($mysqlc,$q);
  ...
  <section>
    <div class="container">
      <div class="user signinBx">
        ...
        <div class="formBx">
          <form action="deletecureve.php"  method="post" enctype="multipart/form-data">
            <h2>Delete current Event</h2>
