...
if(isset($_POST['sid'])){
$sid=$_POST["sid"];
$q="select * from `studentrequest` where `sid`='$sid'";
...
$name=$ar["name"];
$edes=$ar['edes'];
$qe="select `email` from `studentsdata` where id='$sid'";
	$que=mysqli_query($mysqlc,$qe);
    $sub1="CETA EVENT REQUEST INFO";
    $body1="Requested EVENT NAME ".$name." Your request has been approved ";
	$res1=mysqli_fetch_array($que);
    $headers1="From: battinahimaja276@gmail.com";
	$to1=$res1["email"];
    mail($to1,$sub1,$body1,$headers1);
	$q="delete from `studentrequest` where sid='$sid'";
...
if(isset($_POST['add'])){
	...
	$edes=$_POST['edes'];
	$evenue=$_POST['evenue'];
	$qt="select `email` from `studentsdata`";
	$qu=mysqli_query($mysqlc,$qt);
      ...
      <div class="user signinBx">
        ...
        <div class="formBx">
          <form action="aeu2.php"  method="post" enctype="multipart/form-data">
            <h2>Add Student Requested Event</h2>
			
            <input type="file" name="userfile" placeholder="event file" value="<?php echo $link ?>"/><br>
			Event name<input type="text" name="ename" value="<?php echo $name ?>" /><br>
			Event type<pre> </pre>
			<select name="etype" value="<?php echo $type ?>">
			<option><?php echo $type ?></option>
			
			</select>
            <br><br>
			Event des<input type="text" name="edes" value="<?php echo $edes?>"/><br>
